...
def modify_master_password():
    get_rds().modify_master_user_password('mypostgresdb', 'mynewpassword')

def take_backup():
    tags = [{'Key': 'Name', 'Value': 'MyFirstSnapshot'}]
    get_rds().take_backup_of_db_instance('mypostgresdb', 'myveryfirstsnapshot', tags)

def restore_db():
    get_rds().restore_db_from_backup('mypostgresdbfromsnapshot', 'rds:mypostgresdb-2020-03-02-02-17')
